...
import backend.src.auth.schemas as auth_schemas
...
async def verify_email(verification_info: auth_schemas.UserCreate, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
...
async def verify_code(verify_code: auth_schemas.CodeVerify, db: Session = Depends(get_db)):
...
def verify_code_for_reset_password(code_info: auth_schemas.CodeVerify, db: Session=Depends(get_db)):
...
async def reset_password(email: EmailStr, updated_pwd: auth_schemas.ResetPasswordIn, db: Session=Depends(get_db)):
